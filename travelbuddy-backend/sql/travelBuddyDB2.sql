...
DROP TABLE IF EXISTS tbl_member_buddy_data;
...
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='FAQ QnA 유형 테이블';
...
CREATE TABLE IF NOT EXISTS tbl_authority (
	                                         ...
	                                         authority_code_name VARCHAR(20) NOT NULL COMMENT '권한이름',
...
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='회원 계정 테이블';
...
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='인증 테이블';
...
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='공지 테이블';
...
CREATE TABLE IF NOT EXISTS tbl_member_buddy_data (
	                                                 member_buddy_code INT NOT NULL AUTO_INCREMENT COMMENT '회원버디코드',
	                                                 member_code INT NOT NULL COMMENT '회원코드',
	                                                 buddy_code INT NOT NULL COMMENT '버디코드',
	                                                 PRIMARY KEY (member_buddy_code),
	                                                 FOREIGN KEY (member_code) REFERENCES tbl_account(member_code) ON DELETE CASCADE,
	                                                 FOREIGN KEY (buddy_code) REFERENCES tbl_buddy(buddy_code) ON DELETE CASCADE
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='회원 버디 데이터 테이블';



CREATE TABLE IF NOT EXISTS tbl_question_naire_theme (
...
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='질문지 테마 테이블';
...
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='질문지 테이블';
...
CREATE TABLE IF NOT EXISTS tbl_schedule (
	                                        ...
	                                        region_code INT NOT NULL COMMENT '지역코드',
	                                        question VARCHAR(100) NOT NULL COMMENT '질문지',
	                                        sche_date DATE NOT NULL COMMENT '여행날짜',
	                                        sche_list VARCHAR(255) NOT NULL COMMENT '생성된스케줄',
	                                        sche_start_date TIME NOT NULL COMMENT '여행시작시간',
	                                        sche_end_date TIME NOT NULL COMMENT '여행종료시간',
	                                        accom VARCHAR(30) NOT NULL COMMENT '여행숙소',
...
CREATE TABLE IF NOT EXISTS tbl_member_schedule_data (
	                                                    member_sche_code INT NOT NULL AUTO_INCREMENT COMMENT '회원스케줄코드',
	                                                    member_code INT NOT NULL COMMENT '회원코드',
	                                                    sche_code INT NOT NULL COMMENT '스케줄넘버',
	                                                    PRIMARY KEY (member_sche_code),
	                                                    FOREIGN KEY (member_code) REFERENCES tbl_account(member_code) ON DELETE CASCADE,
	                                                    FOREIGN KEY (sche_code) REFERENCES tbl_schedule(sche_code) ON DELETE CASCADE
) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='회원 스케줄 데이터 테이블';
...
-- tbl_faq
INSERT INTO tbl_faq (fq_type_code, faq_title, faq_contents, faq_at) VALUES
	                                                                    (4, '비밀번호를 잊었어요', '비밀번호를 재설정하려면 이메일을 통해 새 비밀번호를 발송해드립니다.', 'N'),
	                                                                    (4, '회원 탈퇴 방법', '회원 탈퇴는 개인정보 보호 정책에 따라, 탈퇴 후 30일 이내에는 복구가 불가능합니다.', 'Y'),
	                                                                    (1, '버디 매칭은 어떻게 하나요?', '버디 배칭은 버디매칭 게시판을 들어가서 신청 버튼을 눌러주세요.', 'N');



-- tbl_Authority
INSERT INTO tbl_authority (authority_code, authority_code_name) VALUES
	                                                                (1, '관리자'),
	                                                                (2, '일반 사용자');
...
-- tbl_verification
INSERT INTO tbl_verification (verification_code, verification_number , is_verified , verification_time , member_code) VALUES
	                                                                                                                      (1, 1234, false, '2024-11-10 12:00:00', 1001),
	                                                                                                                      (2, 5678, false, '2024-11-10 12:05:00', 1002),
	                                                                                                                      (3, 2468, TRUE, '2024-11-10 12:10:00', 1003);

-- tbl_region
INSERT INTO tbl_region (region_code, region_name, region_description, region_img, region_thumbnail_img) VALUES
	                                                                                                        (101, '서울', '서울은 대한민국의 수도로, 다양한 문화와 관광지로 유명합니다.', 'seoul.jpg', 'seoul_thumb.jpg'),
	                                                                                                        (102, '부산', '부산은 해운대, 광안리 등 해변이 유명한 도시입니다.', 'busan.jpg', 'busan_thumb.jpg'),
	                                                                                                        (103, '제주', '제주는 자연경관이 아름다운 섬으로 유명합니다.', 'jeju.jpg', 'jeju_thumb.jpg');

-- tbl_notice
INSERT INTO tbl_notice (notice_code, notice_title, notice_contents, notice_create, notice_count, notice_img, notice_at) VALUES
...
-- tbl_qna
INSERT INTO tbl_qna (qna_code, fq_type_code, qna_title, qna_contents, qna_create) VALUES
	                                                                                  (1, 1, '회원가입 방법에 대해 알려주세요', '회원가입을 위한 이메일 주소와 비밀번호 설정이 필요합니다.', NOW()),
	                                                                                  (2, 2, '결제 오류 발생시 어떻게 하나요?', '결제 오류 발생 시 고객센터를 통해 처리할 수 있습니다.', NOW()),
...
-- tbl_buddy
INSERT INTO tbl_buddy (region_code, buddy_type_code, buddy_title, buddy_contents, buddy_create, buddy_status, buddy_img, buddy_count, buddy_at, buddy_apply) VALUES
	                                                                                                                                                             (101, 1, '서울에서의 만남', '서울 지역에서 함께 할 여행 파트너를 찾고 있습니다.', NOW(), 'N', 'image1.jpg', 150, 'N', 'Y'),
	                                                                                                                                                             (102, 2, '부산 여행 모집', '부산 지역에서 일주일간 함께 여행할 사람을 모집합니다. 경험자 우대.', NOW(), 'N', 'image2.jpg', 230, 'N', 'Y'),
	                                                                                                                                                             (101, 1, '서울에서의 힐링', '서울에서 편안하게 쉴 수 있는 여행 동반자를 찾습니다.', NOW(), 'Y', 'image3.jpg', 75, 'Y', 'N');

-- tbl_buddy_match_data
...
INSERT INTO tbl_question_naire_theme (theme_code, question_theme) VALUES
	                                                                  (1, '힐링'),
	                                                                  (2, '관광/문화'),
	                                                                  (3, '활동'),
	                                                                  (4, '식사');
...
-- tbl_questionnaire
INSERT INTO tbl_questionnaire (quest_code, theme_code, question) VALUES
...
-- tbl_questionnaire
INSERT INTO tbl_answer (answer_code, quest_code, answer) VALUES
...
-- tbl_member_answer
INSERT INTO tbl_member_answer (member_answer_code, quest_code, answer_code, member_code) VALUES
	                                                                                         (1, 1, 1, 1001),
	                                                                                         (2, 2, 2, 1002),
	                                                                                         (3, 3, 3, 1003);
...
-- tbl_schedule
INSERT INTO tbl_schedule (sche_code, region_code, question, sche_date, sche_list, sche_start_date, sche_end_date, accom, accom_img, travel_time, sche_time, member_answer_code) VALUES
	                                                                                                                                                                                (101, 101, '서울의 명소를 알려주세요', '2024-12-01', '서울 타워, 경복궁, 인사동', '09:00:00', '18:00:00', '호텔 A', 'hotelA.jpg', '2시간', '6시간',1),
	                                                                                                                                                                                (102, 102, '부산에서 무엇을 할 수 있을까요?', '2024-12-02', '해운대, 광안리 해수욕장, 부산 타워', '08:00:00', '19:00:00', '호텔 B', 'hotelB.jpg', '3시간', '7시간',2),
	                                                                                                                                                                                (103, 101, '서울에서의 자유여행', '2024-12-05', '홍대, 명동, 남산', '10:00:00', '17:00:00', '호텔 C', 'hotelC.jpg', '2시간', '5시간',3);

-- tbl_MemberScheduleData
INSERT INTO tbl_member_schedule_data (member_sche_code, member_code, sche_code) VALUES
	                                                                                (1, 1001, 101),
	                                                                                (2, 1002, 102),
	                                                                                (3, 1003, 103);

-- tbl_member_buddy_data
INSERT INTO tbl_member_buddy_data (member_code, buddy_code) VALUES
	                                                            (1001, 1),
	                                                            (1002, 2),
	                                                            (1003, 3)